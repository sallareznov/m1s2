...
public class ShiftOrAlgorithm extends Algorithm {
	...
	/**
	 * pre-traite l'algorithme en initialisant les vecteurs des lettres de l'alphabet
	 * @param strand le brin a rechercher
	 * @param genome le genome
	 */
	private void preTreat(Strand strand, Genome genome) {
		...
		for (int i = 0 ; i < alphabet.getSize() ; i++) {
			...
			final Integer[] currentLetterVector = new Integer[strandString.length()];
			for (int j = 0 ; j < strandString.length() ; j++) {
				currentLetterVector[j] = (strandString.charAt(j) == letter) ? 1 : 0;					
			}
	 ...
	 * @param strandBases le tableau de bases du brin a rechercher
	 * @param genomeBases le tableau de bases du genome
	 */
	 ...
	 * @param nbColumn l'indice de la prochaine colonne
	 * @param currentColumn la colonne courante
	 * @param genomeBase la base du genome etudie
	 * @param strandSize la taille du brin
	 */
	private void calculateNextStep(int nbColumn, int[] currentColumn, Base genomeBase, int strandSize) {
		for (int i = currentColumn.length - 1; i >= 1; i--) {
			currentColumn[i] = currentColumn[i - 1];
		}
		currentColumn[0] = 1;
		final Integer[] letterVector = lettersVectors.get(genomeBase.getWording());
	 ...
	 */
	private StrandOccurences retrieveOccurences(Base[] strandBases, Base[] genomeBases) {
	...
	private StrandOccurences findRepetitiveWord(Strand strand, Genome genome) {
		...
		for (int i = 1 ; i < genomeBases.length ; i++) {
			...
			calculateNextStep(i, currentColumn, currentBaseOfTheGenome, strandBases.length);
		}
