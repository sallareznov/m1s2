...
public class KarpRabinAlgorithm extends Algorithm {
	...
	for (final Base base : bases) {
	    finalHash += lettersValues.get(base.getWording()) * Math.pow(alphabetLength, currentCoeff);
	    currentCoeff--;
	}
	return finalHash;
    }
	...
	for (int i = 0; i < genome.getSize() - strand.getSize() + 1; i++) {
	    System.arraycopy(genome.getBases(), i, basesToCompare, 0, strand.getSize());
	    final int basesHash = hash(genome.getAlphabet(), basesToCompare);
	    incrNbComparisons();
	    if ((basesHash == strandHash) && (occurenceFound(basesToCompare, strand.getContent()))) {
